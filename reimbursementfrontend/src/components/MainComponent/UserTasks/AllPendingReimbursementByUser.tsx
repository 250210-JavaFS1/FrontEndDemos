...
export const AllPendingReimbursementByUser: React.FC = () => {
    ...
    useEffect(() => {
        ...
        getAllPendingReimbursementByUser(user);
    ...
    const getAllPendingReimbursementByUser = async (user:User) => {
        try {
            const response = await axios.post("http://localhost:8080/reimbursements/user/pending", user)
    ...
    return (
        <Container className="d-flex flex-column align-items-center mt-3">
            <h3>All Pending Reimbursements view: </h3>
                    ...
                    {reimbursements.map((reimbursement: Reimbursement) => {
                        return (
                            <tr key={reimbursement.reimbId}>
                                ...
                                <td>
                                    <Button variant="outline-success" onClick={()=>{onEdit(reimbursement)}}>Edit</Button>
                                    <Button variant="outline-danger" className="ml-2" onClick={()=>{onDelete(reimbursement)}}>Delete</Button>
                                </td>
                            </tr>
                        );
                    })}
